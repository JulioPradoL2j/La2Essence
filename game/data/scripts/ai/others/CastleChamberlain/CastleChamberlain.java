...
import net.sf.l2jdev.gameserver.model.siege.Castle.CastleFunction;
...
import net.sf.l2jdev.gameserver.network.serverpackets.ExShowCropInfo;
import net.sf.l2jdev.gameserver.network.serverpackets.ExShowCropSetting;
import net.sf.l2jdev.gameserver.network.serverpackets.ExShowManorDefaultInfo;
import net.sf.l2jdev.gameserver.network.serverpackets.ExShowSeedInfo;
import net.sf.l2jdev.gameserver.network.serverpackets.ExShowSeedSetting;
...
public class CastleChamberlain extends Script
{
	...
	private static void funcReplace(Castle castle, NpcHtmlMessage html, int func, String str)
	{
	...
	private static final int getFunctionFee(int func, int level)
	{
		int fee = 0;
		switch (func)
		{
	...
	private static final long getFunctionRatio(int func)
	{
		long ratio = 0;
		switch (func)
		{
	...
	private static final int getDoorUpgradePrice(int type, int level)
	{
		int price = 0;
		switch (type)
		{
	...
	private static final int getTrapUpgradePrice(int level)
	{
		int price = 0;
		switch (level)
		{
	...
	private static final boolean isOwner(Player player, Npc npc)
	{
