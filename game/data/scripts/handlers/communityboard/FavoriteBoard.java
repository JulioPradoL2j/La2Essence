...
public class FavoriteBoard implements IParseBoardHandler
{
	...
	public boolean onCommand(String command, Player player)
	{
		...
		if (command.startsWith("_bbsgetfav"))
		{
			...
			try (Connection con = DatabaseFactory.getConnection(); PreparedStatement ps = con.prepareStatement(SELECT_FAVORITES))
			{
		...
		else if (command.startsWith("bbs_add_fav"))
		{
			...
			if (bypass != null)
			{
				...
				try (Connection con = DatabaseFactory.getConnection(); PreparedStatement ps = con.prepareStatement(ADD_FAVORITE))
				{
					ps.setInt(1, player.getObjectId());
		...
		else if (command.startsWith("_bbsdelfav_"))
		{
			...
			try (Connection con = DatabaseFactory.getConnection(); PreparedStatement ps = con.prepareStatement(DELETE_FAVORITE))
			{
				ps.setInt(1, player.getObjectId());
