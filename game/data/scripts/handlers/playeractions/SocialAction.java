...
import net.sf.l2jdev.gameserver.network.serverpackets.ExAskCoupleAction;
...
public class SocialAction implements IPlayerActionHandler
{
	...
	private static boolean useSocial(Player player, int id)
	{
		if (player.isFishing())
		{
		...
		{
			player.broadcastPacket(new net.sf.l2jdev.gameserver.network.serverpackets.SocialAction(player.getObjectId(), id));
	...
	private static void useCoupleSocial(Player player, int id)
	{
		if (player == null)
		{
			return;
