...
public class Fan implements IAffectScopeHandler
{
	...
	public void forEachAffected(Creature creature, WorldObject target, Skill skill, Consumer<? super WorldObject> action)
	{
		...
		final Predicate<Creature> filter = c -> {
			...
			if ((c.isDead() && (targetType != TargetType.NPC_BODY) && (targetType != TargetType.PC_BODY)) || (Math.abs(LocationUtil.calculateAngleFrom(creature, c) - (headingAngle + fanStartAngle)) > fanHalfAngle))
			{
				return false;
			}
