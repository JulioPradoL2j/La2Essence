...
public class SellBuffsManager implements IXmlReader
{
	...
	public static final String HTML_FOLDER = "data/html/mods/SellBuffs/";
	...
	public void load()
	{
		if (SellBuffsConfig.SELLBUFF_ENABLED)
		{
			...
			LOGGER.info(this.getClass().getSimpleName() + ": Loaded " + ALLOWED_BUFFS.size() + " allowed buffs.");
		}
	}
	...
	{
		NodeList node = document.getDocumentElement().getElementsByTagName("skill");

		for (int i = 0; i < node.getLength(); i++)
		{
			Element elem = (Element) node.item(i);
			int skillId = Integer.parseInt(elem.getAttribute("id"));
	...
	{
		String html = HtmCache.getInstance().getHtm(player, "data/html/mods/SellBuffs/" + (player.isSellingBuffs() ? "BuffMenu_already.html" : "BuffMenu.html"));
	...
	public void sendBuffMenu(Player player, Player seller, int index)
	{
		if (seller.isSellingBuffs() && !seller.getSellingBuffs().isEmpty())
		{
			...
			html = html.replace("%list%", this.buildBuffMenu(seller, index));
			CommunityBoardHandler.separateAndSend(html, player);
		}
	}
	...
	public String buildBuffMenu(Player seller, int index)
	{
		int nextIndex = -1;
		int previousIndex = -1;
		int emptyFields = 0;
		StringBuilder sb = new StringBuilder();
		List<SellBuffHolder> sellList = new ArrayList<>();
		int count = 0;
		...
		for (SellBuffHolder holderx : sellList)
		{
			Skill skill = seller.getKnownSkill(holderx.getSkillId());
			if (skill == null)
			{
				emptyFields++;
			}
			...
			{
				ItemTemplate item = ItemData.getInstance().getTemplate(SellBuffsConfig.SELLBUFF_PAYMENT_ID);
				sb.append("<tr>");
				sb.append("<td fixwidth=\"20\"></td>");
				...
				sb.append("<td align=center> " + FormatUtil.formatAdena(holderx.getPrice()) + " <font color=\"LEVEL\"> " + (item != null ? item.getName() : "") + "</font> </td>");
	...
	public String buildEditMenu(Player player)
	{
		StringBuilder sb = new StringBuilder();
		sb.append("<table border=0 cellpadding=0 cellspacing=0 background=\"L2UI_CH3.refinewnd_back_Pattern\">");
		sb.append("<tr><td><br><br><br></td></tr>");
		...
		else
		{
			for (SellBuffHolder holder : player.getSellingBuffs())
			{
				...
				if (skill != null)
				{
					...
					sb.append("<td align=left>" + skill.getName() + (skill.getLevel() > 100 ? "<font color=\"LEVEL\"> + " + skill.getLevel() % 100 + "</font></td>" : "</td>"));
					sb.append("<td align=center>" + (skill.getLevel() > 100 ? SkillData.getInstance().getMaxLevel(skill.getId()) : skill.getLevel()) + "</td>");
					sb.append("<td align=center> " + FormatUtil.formatAdena(holder.getPrice()) + " </td>");
	...
	private String buildSkillMenu(Player player, int index)
	{
		...
		{
			if (ALLOWED_BUFFS.contains(skill.getId()) && !this.isInSellList(player, skill))
			{
				count++;
		...
		{
			for (Skill skillx : skillList)
			{
				sb.append("<tr>");
				sb.append("<td fixwidth=\"20\"></td>");
				sb.append("<td align=center><img src=\"" + skillx.getIcon() + "\" width=\"32\" height=\"32\"></td>");
				sb.append("<td align=left>" + skillx.getName() + (skillx.getLevel() > 100 ? "<font color=\"LEVEL\"> + " + skillx.getLevel() % 100 + "</font></td>" : "</td>"));
				sb.append("<td align=center>" + (skillx.getLevel() > 100 ? SkillData.getInstance().getMaxLevel(skillx.getId()) : skillx.getLevel()) + "</td>");
				sb.append("<td align=center><edit var=\"price_" + skillx.getId() + "\" width=120 type=\"number\"></td>");
				sb.append("<td align=center fixwidth=\"50\"><button value=\"Add Buff\" action=\"bypass -h sellbuffaddskill " + skillx.getId() + " $price_" + skillx.getId() + "\" width=\"85\" height=\"26\" back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
				sb.append("</tr>");
				sb.append("<tr><td><br><br></td></tr>");
			}
	...
	{
		return SellBuffsManager.SingletonHolder.INSTANCE;
	}
