...
public class RequestPurchaseLimitCraftItem extends ClientPacket
{
	...
	protected void readImpl()
	{
		...
		this.readInt();
		this.readInt();
		this._product = LimitShopCraftData.getInstance().getProduct(this._productId);
	}
		...
		if (player != null)
		{
			if (this._product != null)
			{
				if (this._amount < 1 || this._amount > 10000)
				{
					...
					player.sendPacket(new ExPurchaseLimitShopItemResult(false, 4, this._productId, 0, Collections.emptyList()));
				}
				...
				else if (player.getLevel() >= this._product.getMinLevel() && player.getLevel() <= this._product.getMaxLevel())
				{
					if (!player.hasItemRequest() && !player.hasRequest(PrimeShopRequest.class))
					{
							...
							if (this._product.getIngredientIds()[i] != 0)
							{
								if (this._product.getIngredientIds()[i] == 57)
								{
									...
									if ((amountxxxx < 1L) || (player.getAdena() < amountxxxx))
									{
										player.sendPacket(SystemMessageId.INCORRECT_ITEM_COUNT_2);
										player.removeRequest(PrimeShopRequest.class);
										player.sendPacket(new ExPurchaseLimitShopItemResult(false, 4, this._productId, remainingInfo, Collections.emptyList()));
										return;
									}
								}
						...
						for (int ixx = 0; ixx < this._amount; ixx++)
						{
							...
							{
								rewards.put(0, new LimitShopRandomCraftReward(this._product.getProductionId(), (int) this._product.getCount(), 0));
								Item item = player.addItem(ItemProcessType.CRAFT, this._product.getProductionId(), this._product.getCount(), this._product.getEnchant(), player, true);
							...
							{
								rewards.put(0, new LimitShopRandomCraftReward(this._product.getProductionId2(), (int) this._product.getCount2(), 1));
								Item item = player.addItem(ItemProcessType.CRAFT, this._product.getProductionId2(), this._product.getCount2(), this._product.getEnchant(), player, true);
						...
						player.sendPacket(new ExPurchaseLimitShopItemResult(true, 4, this._productId, Math.max(remainingInfo - this._amount, 0), rewards.values()));
						player.sendItemList();
