...
public class BlowfishEngine {
   ...
   public void processBlock(Buffer data, int offset) throws IOException {
      ...
      } else if (offset + 8 > data.limit()) {
         throw new IOException("input buffer too short");
      } else {
         if (this.encrypting) {
            this.encryptBlock(data, offset);
         } else {
            this.decryptBlock(data, offset);
         }
      }
   }
   ...
   private void processTable(int xl, int xr, int[] table) {
      ...
      for (int s = 0; s < size; s += 2) {
         ...
         for (int i = 1; i < 16; i += 2) {
            xr ^= this.func(xl) ^ this.P[i];
            xl ^= this.func(xr) ^ this.P[i + 1];
         }
   ...
   private void encryptBlock(Buffer data, int index) {
      ...
      xr ^= this.P[17];
      this.bits32ToBytes(xr, data, index);
      this.bits32ToBytes(xl, data, index + 4);
   }
   ...
   protected void bits32ToBytes(int in, Buffer data, int offset) {
